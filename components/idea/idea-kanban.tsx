...
const IdeaKanban = () => {
    ...
    const [syncedIdeaData, setSyncedIdeaData] = useState<unknown>(undefined);

    if (ideaData !== syncedIdeaData) {
        setSyncedIdeaData(ideaData);
...
p-2 px-3 transition-colors min-h-0`,
            ...
            <IdeaDialog
                key={`${showIdeaDialog ? "open" : "closed"}-${selectedIdea?.id ?? "new"}-${selectedColumnId}`}
                open={showIdeaDialog}
                onOpenChange={(open) => {
