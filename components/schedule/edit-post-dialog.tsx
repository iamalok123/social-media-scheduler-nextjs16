...
import IdeasList, { type Idea } from "./ideas-list";
...
function getPostDate(post: EditPostDialogProps["post"]) {
    return post ? new Date(post.scheduledDate) : new Date();
}

function getPostTime(post: EditPostDialogProps["post"]) {
    if (!post) return "";
...
export function EditPostDialog({
    ...
    const [time, setTime] = React.useState<string>(getPostTime(post));
    const [selectedRightTab, setSeletedRightTab] =
        React.useState<ActionTabType | null>(null);
                            ...
                            <div className="flex items-center gap-px">
                                {rightTabs.map((tab) => (
                                    <Button
                                        ...
                                        className={cn(
                                            !selectedRightTab && "size-8",
                                            "",
                                        )}
                                        onClick={() =>
                                            handleSelectRightTab(tab.id)
                                        }
                                    >
                                        <tab.icon className="h-4 w-4" />
                                        <span
                                            className={cn(
                                                !selectedRightTab && "hidden",
                                            )}
                                        >
                                            {tab.label}
                                        </span>
